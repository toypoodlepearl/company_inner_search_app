...
def initialize_retriever():
    ...
    try:
        ...
        logger.info(f"読み込み完了: {len(docs_all)}件のドキュメント")
        ...
        logger.info(f"テキスト分割完了: {len(splitted_docs)}チャンク")
...
def load_data_sources():
    ...
    for web_url in ct.WEB_URL_LOAD_TARGETS:
        try:
            ...
            print(f"Webページ読み込みエラー ({web_url}): {str(e)}")
            # Webページ読み込みに失敗してもアプリは継続
            continue
